module CompletedTestsHelper
  ...
  def progress(completed_test)
    "Progress: #{completed_test.current_question_index} / #{questions_total(completed_test)}"
  end
  ...
  def completed_test_result(completed_test)
    ...
    message = "Test #{completed_test_status(completed_test)}, you result #{result_percent}%"
